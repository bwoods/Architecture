...
impl<State: Reducer> Store<State> {
    ...
    pub fn into_inner(self) -> State {
        drop(self.actions); // ends the `Selector` while-loop
        self.handle.join().unwrap()
    }
}

impl<State: Reducer> Effects for Store<State> {
...
pub mod tests {
    ...
    fn test_into_inner() {
        ...
        let _result = store.into_inner(); // ensuring that `into_inner` compiles
    }
}
