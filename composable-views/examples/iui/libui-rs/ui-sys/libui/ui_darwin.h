...
#define uiDarwinControlDefaultHuggingPriority(type, handlefield) \
	...
	{ \
		return [(id)type(c)->handlefield contentHuggingPriorityForOrientation:orientation]; \
	}
#define uiDarwinControlDefaultSetHuggingPriority(type, handlefield) \
	...
	{ \
		[(id)type(c)->handlefield setContentHuggingPriority:priority forOrientation:orientation]; \
	}
